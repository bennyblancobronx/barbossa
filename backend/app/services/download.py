...
class DownloadService:
    ...
    async def _import_trusted_source(
        self,
        path: Path,
        source: str,
        source_url: str,
        ...
        """Import album from trusted source (Qobuz, Bandcamp purchases).

        Skips beets identification - trusts embedded metadata from source.
        Uses exiftool to read tags, then moves to library.
        """
        # Extract metadata from files (Qobuz embeds excellent tags)
        tracks_metadata = await self.exiftool.get_album_metadata(path)

        if not tracks_metadata:
            raise ImportError(f"No audio files found in {path}")

        # Get artist/album from first track's embedded tags
        first_track = tracks_metadata[0]
        artist = first_track.get("artist") or self.beets._extract_artist_from_path(path)
        album_title = first_track.get("album") or path.name

        # Parse from folder name as fallback
        if artist == "Unknown Artist" or not artist:
            folder_info = self.beets._parse_folder_name(path.name)
            artist = folder_info.get("artist") or "Unknown Artist"
            album_title = folder_info.get("album") or album_title
        ...
        if existing:
            ...
            # Replace with higher quality - move to library
            ...
            return await self.import_service.replace_album(existing.id, library_path, tracks_metadata)

        # Move to library using beets (organizes files)
        library_path = await self.beets.import_with_metadata(
            path,
            artist=artist,
            album=album_title,
        ...
        # Re-extract metadata after move
        tracks_metadata = await self.exiftool.get_album_metadata(library_path)

        # Import to database with full confidence (trusted source)
        return await self.import_service.import_album(
            ...
            imported_by=user_id,
            confidence=1.0  # Trusted source = full confidence
        )
